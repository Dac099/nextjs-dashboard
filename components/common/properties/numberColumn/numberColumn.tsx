...
import type { ColumnData, GroupData, ItemData, ItemValue } from '@/utils/types/views';
...
export const NumberColumn = ({ value, item, column }: Props) => {
        ...
        if (e.type === 'blur' || (e.type === 'keydown' && (e as React.KeyboardEvent).key === 'Enter')) {
            if (!isNaN(inputValue)) {
                try{
                    ...
                    const itemToShow = await updateNumberColumn(item, column, newItemValue);                
